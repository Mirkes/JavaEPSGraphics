...
/**
 * Class MyEPSGraphics of package epsgraphics serves to save image in Encapsulated Post Script (eps) format.
 *
 * Using of this class is simple and usual:
 *
 * To save component comp (any descender of class Component) it is necessary to add following code:
 * 
 *    MyEPSGraphics mg = new MyEPSGraphics(comp.width, comp.height, application name", 
 *        "Copyright text", BufferedWriter(new FileWriter("file name.eps")));
 *    comp.paint(mg);
 *    mg.close();
 *
 * Methods of creation and closing of MyEPSGraphics create, write and close file (or other writer). 
 * These methods can throw IOExeption.
 *
 * Debug information is addressed to developer who want to improve package. To use debug information is necessary 
 * to switch on boolean field inDebug in EPSStorage. To do this method setDebug(boolean debug) is added. In example 
 * above it is enough to add <code>mg.setDebug(true);</code>
 *
 * Usage of short spelling PS commands now is not optional. The usage of such trick help to obtain shorter files. 
 * Since list of shortage command includes in file I decide use abbreviated and redefined commands permanently. 
 * The real list of short command is described in EPSStorage. To correct usage of short command each added to storage 
 * string analysed by following rule:
 *
 ...
 * 3. The found part is checked in map of abbreviation.
 *
 * If you use the command which located not in the end of string you must call setCommand method of storage to check 
 * this command. If you do not call setCommand then this command can be not included in the result file.
 *
 * There are two mode to work with fonts in this class:
 * 1. Standard mode is using Java defined Postscript names of fonts. The real existence of fonts on computer where 
 *    file will be used in this case is the problem of user. This mode can be set by using 
 *    <code>encapsulateFonts(false)</code>.
 * 2. Encapsulate font to EPS file. This mode can be set by using <code>encapsulateFonts(true)</code>. In this mode 
 *    all used fonts are inserted to EPS file and this file can be used without any problem but size of file in this 
 *    case will be greater.
 *
 * Since the PS does not work with transparency I exclude composite. But I like to use Alpha to change colors. 
 * For this purpose I proceed alpha composition of new color with background color in setColor method. To implement 
 * full transparency algorithm we must create the png (bmp, gif, etc.) image and then convert it to EPS. The result 
 * of such work will not be a pure EPS file and converters of png, jpeg and bmp to EPS exist.
 *
 * Since the PS does not work with gradient painting I not support Paint. It is possible to implement this property 
 * latter when I understand the easy and clear way to do it.
 *  
